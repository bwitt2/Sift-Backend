...
class Article(messages.Message):
  ...
  summarized_article = messages.StringField(7, required = True)
...
class ArticleModel(ndb.Model):
  ...
  summarized_article = ndb.StringProperty(required = True)
  full_article = ndb.StringProperty(required = True)
